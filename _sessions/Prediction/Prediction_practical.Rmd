...
2. Using `train()` fit a regression model called `grad_glm` predicting `Grad_Rate` as a function of all features except for `college_name`

- For the `form` argument, use `Grad_Rate ~ . -college_name`
...
```{r, eval = FALSE, echo = TRUE}
grad_rf <- train(form = XX ~ . -XX,   # Predict grad and exclude college_name
                 data = XX,
                 method = "XX",
                 trControl = XX,
