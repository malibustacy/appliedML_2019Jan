...
dt_pred <- predict(hwy_rpart, newdata = data_test)
...
6. Using `predict()` save the fitted values of `grad_rpart` object as `dt_fit`:
...
```{r}
price_glm <- train(form = price ~ . - id - date,
                  data = house_train,
                  method = "glm",
                  trControl = ctrl_none)
```
...
4. Using `train()`, fit a decision tree model called `price_rpart` predicting `price` (using the same features as above).
...
```{r, echo = TRUE, eval = FALSE}
price_rpart <- train(form = XX ~ . - XX - XX,
                  data = XXX,
                  method = "XX",
                  trControl = XX,
                  tuneGrid = expand.grid(cp = XX))   # Set complexity parameter
...
```{r}
price_rpart <- train(form = price ~ . - id - date,
                  data = house_train,
                  method = "rpart",
                  trControl = ctrl_none,
                  tuneGrid = expand.grid(cp = .01))   # Set complexity parameter
...
6. Using `predict()` save the fitted values of `price_rpart` object as `dt_fit`:
...
```{r}
dt_fit[1:10]   # Only first 10 values
summary(dt_fit)
hist(dt_fit)
```
...
```{r}
price_rf <- train(form = price ~ . - id - date,   # Predict price and exclude id and date
                 data = house_train,
                 method = "rf",
                 trControl = ctrl_none,
                 tuneGrid = expand.grid(mtry = 2))  # Set number of features randomly selected
