...
2. Open a new R script. At the top of the script, using comments, write your name and the date. Save it as a new file called `Fitting_College_practical.R` in the `2_Code` folder.  
3. Using `library()` load the set of packages for this practical listed in the packages section above.

```{r, eval = FALSE, echo = TRUE}
## NAME
## DATE
## Fitting Practical - College Graduation Rates Prediction
...
- For the `form` argument, use `Grad.Rate ~ . - id - date`
- For the `data` argument, use  `college_train` in the data argument
...
```{r, eval = FALSE, echo = TRUE}
grad_rf <- train(form = XX ~ . - XX - XX,   # Predict grad and exclude id and date
                 data = XX,
                 method = "XX",
                 trControl = XX,
...
7. If someone came to you and asked "If I use your model in the future to predict the priceuation rate of a new college, how accurate do you think it would be?", what would you say?
