...
hwy_rf <- train(form = hwy ~ year + cyl + displ,
                ...
                method = "rf",
                trControl = train_control,
                tuneGrid = expand.grid(mtry = 2))   # Set mtry
...
1. Now it's time to compare your model fits to the true values, we'll start by defining a vector called `truth_train`!

```{r}
# Define Grad_Rate as the 'truth'

criterion_train <- college_train$Grad_Rate
```
...
postResample(pred = glm_fit,    # Fitted values 
             obs = criterion_train) # True values
```
...
accuracy <- tibble(criterion = criterion_train,
                   Regression = glm_fit) %>%
...
6. Using the code below, create a scatterplot showing the relationship between the criterion and the model fits
...
```{r}
# Define the criterion as college_train$Private
...
3. Now it's time to calculate model accuracy, to do this, we will use a new function called `confusionMatrix()`. This function compares model predictions to a 'reference' (in our case, the truth, and returns several summary statistics). In the code below, we'll use `glm_fit` as the model predictions, and our already defined `criterion` as the reference
