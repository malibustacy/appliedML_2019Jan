...
# What is model prediction / testing?

<font size = 5>Model prediction (aka, testing) is the process of accessing a model's predictions on <i>new</i> <high>testing data</high>.</font>

# What is test data? 

<font size = 5>Testing data is a separate, 'hold-out' data set that the model <high>never saw during training</high></font>
...
<font size = 5>Just because a model can <high>fit past data well</high> (high training accuracy), does <i>not</i> necessarily mean that it will <high>predict new data well</high> (high testing accuracy)</font>
...
---
<br><br><br>
<font size = 6>Can you come up with a model that will perfectly fit the training criterion but is worthless in predicting test data?</font><br><br>

.pull-left45[

<br>
<font size=5><high>Training</high> Data</font>
...
<br>
<font size=5><high> Test</high> Data</font>
...
Use models with <high>regularisation</high> terms, which explicitly punish models for being too complex.

Use fitting methods such as <high>cross-validation</high> to find optimal regularisation values.
...
Ideally, you should have two distinct. naturally occurring training and test datasets.
...
Randomly split a dataset into an X% training set and 1-X% testing set.
...
<br><br>
### How Decision Trees are grown

Find splits in data that maximize *Node Purity*
...
To fit a decision tree in caret, use `method = "rpart"`
...
Decision trees have a <high>complexity parameter</high> called <high>cp</high>

This controls how complex (large) trees are allowed to grow

- Small cp (< 0.01) = Complex Trees
- Large cp (> 0.10) = Simple Trees

The best value of cp depends on your dataset!
...
This control show complex (large) trees are allowed to grow
...
```{r, eval = FALSE}
# Fit a random forest with a defined mtry = 4

train(form = income ~ .,
      data = baselers,
      method = "rf",  # Random Forest
      trControl = ctrl,
      tuneGrid = expand.grid(mtry = 4)) # mtry
```
...
The "best" value of mtry depends on your dataset!
...
Testing model predictions with `caret` is very easy.

All you need to do is get a vector of predictions from a new dataframe `newdata` using the `predict()` function:
...
To get predictions, use `predict(mod, newdata = data_test)`
