...
```{r, echo = FALSE, fig.align = 'center', eval = TRUE, out.width = "70%", fig.cap="Wikipedia"}
...
1. Use cross-validation to select optimal model tuning parameters
2. Compare 'standard' regression with lasso and ridge regression
3. Use cross-validation to predict test accuracy
...
#    16571.06    10229.91     -165.30      -82.52    -3958.62     2937.76 
...
ggplot(data = accuracy,
  ...
  labs(title = "Predicting Diamond Prices",
...
# 0 - Baseball player salaries

In this section, we will predict the salaries of baseball players from the `hitters_train` and `hitters_test` datasets. Here's how the first few rows of `hitters_train` should look:
...
1. Set up your training by specifying `ctrl_cv` as 10-fold cross-validaation
...
3. Try plotting your `price_glm` object. What happens? What does this error mean?

4. Print your final model object with `price_glm$finalModel`
...
5. Perint your final regression model coefficients with `coef()`.
...
It's time to fit an optimised regression model with a Ridge penalty!


1. Before we can fit a ridge regression model, we need to specify which values of the lambda penalty parameter we want to try. Using the code below, create a vector called `lambda_vec` which contains 100 values between 0 and 2

```{r, echo = TRUE, eval = FALSE}
# Determine possible values of lambda

lambda_vec <- seq(from = 0, to = 1000, length = 100)
```
...
- Set the formula to `Salary ~ .`
- Set the data to `hitters_train`
- Set the method to `"glmnet"` for regularised regression
- Set the train control argument to `ctrl_cv`
...
3. Plot your `price_ridge` object. What do you see? Which value of the regularisation parameter seems to be the best?

```{r, echo = TRUE, eval = FALSE}
# Plot price_ridge object
plot(XX)
```

4. Print the best value of lambda by running the following code. Does this match what you saw in the plot above?
...
5. What were your final regression model coefficients for the best lambda value? Find them by running the following code:
...
6. How do these coefficients compare to what you found in regular regression? Are they similar? Different?

## C - Lasso Regression

It's time to fit an optimised regression model with a Lasso penalty!

1. Before we can fit a ridge regression model, we need to specify which values of the lambda penalty parameter we want to try. Using the code below, create a vector called `lambda_vec` which contains 100 values between 0 and .5
...
3. Plot your `price_lasso` object. What do you see? Which value of the regularisation parameter seems to be the best?

```{r, echo = TRUE, eval = FALSE}
# Plot price_lasso object
plot(XX)
```
...
6. How do these coefficients compare to what you found in regular regression? Are they similar? Different? Do you notice that any coefficients are now set to exactly 0?

## C - Decision Tree

It's time to fit an optimised decision tree model!!

1. Before we can fit a decision treel, we need to specify which values of the complexity parameter `cp` we want to try. Using the code below, create a vector called `cp_vec` which contains 100 values between 0 and 1

```{r, echo = TRUE, eval = FALSE}
# Determine possible values of cp
...
- Set the train control argument to `ctrl_cv`
- Set the tuneGrid argument such that alpha is 1 (for lasso regression), and with all lambda values you specified in `cp_vec` (we've done this for you below)

```{r, echo = TRUE, eval = FALSE}
# Decision Tree --------------------------

price_dt <- train(form = XX ~ .,
                  data = XX,
                  method = "XX",
                  trControl = XX,
                  tuneGrid = expand.grid(cp = cp_vec))
...
3. Plot your `price_dt` object. What do you see? Which value of the regularisation parameter seems to be the best?

```{r, echo = TRUE, eval = FALSE}
# Plot price_dt object
plot(XX)
```
...
5. Plot your final decision tree using the following code

```{r, echo = TRUE, eval = FALSE}
# Visualise your trees
plot(as.party(price_dt$finalModel)) 
```

6. How do the nodes in the tree compare to the coefficients you found in your regression analyses? Do you see similarities or differences?

## D - Random Forests

It's time to fit an optimised random forest model!
...
3. Plot your `salary_rf` object. What do you see? Which value of the regularisation parameter seems to be the best?
...
4. Print the best value of mtry by running the following code. Does this match what you saw in the plot above?
...
## Estimate prediction accuracy from training folds
...
2. Now do the same with your ridge, lasso, decision tree, and random forest models to get the objects `ridge_pred`, `lasso_pred`, `dt_pred` and `rf_pred`
...
3. Using `postResample()`, calculate the aggregate prediction accuracy for each model using the template below
...
2. Using the same procedure as above, compare models predicting the prices of houses in King County Washington using the `house_train` and `house_test` datasets. Which model does the best and how accurate was it? Was it the same model that performed the best predicting baseball player salaries?

3. Using the same procedure as above, compare models predicting the graduate rate of students from different colleges using the `college_train` and `college_test` datasets. Which model does the best and how accurate was it? Was it the same model that performed the best predicting baseball player salaries?
