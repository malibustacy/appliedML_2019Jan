...
1. Before we can fit a ridge regression model, we need to specify which values of the lambda penalty parameter we want to try. Using the code below, create a vector called `lambda_vec` which contains 100 values spanning a wide range.
...
lambda_vec <- 10 ^ seq(from = -3, to = 3, length = 100)
```
...
# I see that RBI, CAtBat, CHits and Errors are all removed!
```
...
dt_pred <- predict(salary_dt, newdata = hitters_test)
...
5. Which of your models had the best performance in the true training data?

```{r}
# Random forests had the lowest MAE!

 #   RMSE Rsquared      MAE 
 # 237.934    0.633  153.482 
```
