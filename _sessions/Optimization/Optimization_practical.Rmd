...
In this section, we will again predict the prices of houses in King County Washington from the `house_train` and `house_test` datasets. Here's how the first few rows of `house_train` should look:
...
# Carseats data
carseats_train <- read_csv(file = "1_Data/carseats_train.csv")
carseats_test <- read_csv(file = "1_Data/carseats_test.csv")
```
...
```{r}
data_train <- read_csv(file = "1_Data/carseats_train.csv") %>%
  slice(1:15)

data_test <- read_csv(file = "1_Data/carseats_test.csv")
```
...
salary_glm <- train(form = XX ~ . -XX - XX,
                    data = XX,
                    method = "XX",
                    trControl = XX)
```
...
salary_ridge <- train(form = XX ~ . -XX - XX,
                      data = XX,
                      method = "XX",
                      trControl = XX,
                      tuneGrid = expand.grid(alpha = 0,  # Ridge penalty
...
salary_lasso <- train(form = XX ~ . -XX - XX,
                      data = XX,
                      method = "XX",
                      trControl = XX,
...
```{r, echo = TRUE, eval = FALSE}
plot(as.party(price_dt$finalModel))   # Visualise your trees
```
