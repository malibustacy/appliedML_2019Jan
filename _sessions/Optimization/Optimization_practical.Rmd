...
# What was the best lambda value?
...
9. We need to do a little bit of data cleaning before starting. Specifically, we need to delete the id column to make it easier to work with the data: Do this by running the following code:
...
2. Using `train()` fit a regression model called `price_glm` predicting `price` as a function of all features *except* for date (Ask yourself, why do you think we are excluding this?).

- For the `form` argument, use `price ~ . - date`
