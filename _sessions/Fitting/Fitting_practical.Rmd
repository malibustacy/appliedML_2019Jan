...
2. Using the same steps as above, create a regression model `glm_fit` which predicts `Grad.Rate` using all features *except* for that one we don't want (remember, use Y ~ . - XXX to remove a feature XXX)

```{r, echo = TRUE}
Grad.Rate_glm <- train(form = Grad.Rate ~ . - XXX,   # Get rid of that one feature!
                       data = college_train,
                       method = "glm",
                       trControl = ctrl_none)
```
