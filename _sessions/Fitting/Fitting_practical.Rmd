...
competition_results <- tibble(truth = house_train$price,
                              Regression = lm_fit,
...
4. For this practical, we'll use a dataset related to the prices of houses in [King County Washington](https://en.wikipedia.org/wiki/King_County,_Washington) (which you have to thank for Starbucks). The data is stored in `house_train.csv`. Using the following template, load the dataset into R as `house_train`:
