...
house_train <- read_csv(file = "1_Data/violent_crime.csv")
house_train <- read_csv(file = "1_Data/nonviolent_crime.csv")
```
...
1. As always, before you do anything you need to make sure that you separate a hold-out data set for later. Create `pima_train` and `pima_test` using `createDataPartition()` with 80% of cases going into the training set. The criterion is `diabetes`. 

```{r, echo = TRUE}
train_index <- createDataPartition(XX$XX, p = .8, list = FALSE)
...
Ok, now that we have a training set, let's get to work and remove some features.
...
1. Although, this dataset is rather small and rather well curated, test if there are any excessively correlated features using `cor()` and `findCorrelation()` using the code below. 
